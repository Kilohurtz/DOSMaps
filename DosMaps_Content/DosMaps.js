...
$(document).ready(function() {
	...
	$(window).resize(function() {
		console.log("hi");
		document.getElementById("head").innerHTML=($("#perspective").css("width"));
		$("#svg").css("width",$("#perspective").css("width"));
		var width = $("#perspective").css("width");
		
		//updatez(z.cur*-1);
		//updatep(px.cur*-1, py.cur*-1);
		$("#svg").css("height","100%");
		z.cur = 0;
		py.cur = 0;
		px.cur = 0;
	...
	$(".continents").change(function(){
		if ($(this).val() == "world"){
			...
			updatepzA();
		}else if($(this).val() == "north-america"){
			//updatepzA(720,720,0,135);
	...
	var down = false;
	var x, y;
	$('#perspective').mousedown(function(e){
		down = true;
		x = e.clientX;
		y = e.clientY;
	...
	$('#perspective').mouseup(function(e){
		if (down){
			x-=e.clientX;
			y-=e.clientY;			
			updatep(-x,-y);
	...
	$('#perspective').bind('mousewheel', function(e){
...
var r = 0;
function updatez(zoom){
	if (z.cur+zoom<=z.max && z.cur+zoom>=z.min){
		//$("#svg").css("width",($("#svg").width()+zoom*r).toString()+"px");
		$("#svg").css("height",($("#svg").height()+zoom).toString()+"px");
		updatep(-(zoom)/2,-zoom/2);
		z.cur+=zoom;
	}
	
	document.getElementById("head").innerHTML=("z="+z.cur+" px="+px.cur+" py="+py.cur);

}
function updatep(perx,pery){ //move x and y px
	...
	if (px.cur+perx<=px.max && px.cur+perx>=px.min){
		$("#svg").css("margin-left",($("#svg").css("margin-left")*perx).toString()+"px");
		px.cur+=perx;
	}
	
	if (py.cur+pery<=py.max && py.cur+pery>=py.min){
		$("#svg").css("margin-top",($("#svg").css("margin-left")*pery).toString()+"px");
		py.cur+=pery;
	}
...
function pxToWidth(px){
	return px*$("#svg").width()/1000;
}

function pxToWidth(W){
	return W*1000/$("#svg").width();
}